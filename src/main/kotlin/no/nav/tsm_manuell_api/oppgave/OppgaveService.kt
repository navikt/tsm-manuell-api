    ...
    fun opprettGosysOppgave(manuellOppgave: ManuellOppgave): GosysOpprettOppgaveResponse {
        ...
        val gosysOppgaveResponse =
            gosysOppgaveClient.opprettOppgave(gosysOpprettOppgave).fold({ it }) {
                logger.error(
                    "Feil ved opprettelse av Gosys oppgave for sykmelding ${manuellOppgave.sykmeldingRecord.sykmelding.id}: ${it.message}"
                )
                throw RuntimeException("Feil ved opprettelse av Gosys oppgave")
            }
        OPPRETT_OPPGAVE_COUNTER.inc()
        logger.info(
            "Opprettet Gosys oppgave med id ${gosysOppgaveResponse.id} for sykmelding ${manuellOppgave.sykmeldingRecord.sykmelding.id}"
        )
